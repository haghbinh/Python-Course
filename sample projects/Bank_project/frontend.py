...
from backend import BankSystem, Cashier, Customer
...
class BankApp:
    ...
    def create_role_selection(self):
        self.role_var.set("customer")  # Set default selection to "Customer"
        Label(self.registration_window, text="Role: ").grid(row=4, column=0)
        Radiobutton(self.registration_window, text="Customer", variable=self.role_var, value="customer").grid(row=4, column=1, sticky="w")
    ...
    def login(self):
        ...
        if user is None:
            ...
            if self.role_var.get() == "customer":
                self.open_customer_window(user)
            else:
    ...
    def open_customer_window(self, user):
        self.customer_window = Toplevel(self.login_window)
        self.customer_window.title("Customer Window")
        self.customer_window.geometry("300x100")

        Label(self.customer_window, text="Welcome, " + user.first_name + " " + user.last_name).grid(row=0, column=0)

        # Add customer-specific widgets and functionality here
        self.create_profile_button()
        self.create_exit_button(self.customer_window)  # Add exit button

    def create_profile_button(self):
        button = Button(self.customer_window, text="View Profile", command=self.view_profile)
    ...
    def open_cashier_window(self, user):
        ...
        self.cashier_window.geometry("300x100")
    ...
    def create_change_value_button(self):
        button = Button(self.cashier_window, text="Change Value", command=self.change_value)
        button.grid(row=1, column=0, columnspan=2)

    def change_value(self):
        new_value = simpledialog.askinteger("Change Value", "Enter new value:")
        if new_value is not None:
            # Process the new value here
            messagebox.showinfo("Value Changed", "The value has been changed to: " + str(new_value))
